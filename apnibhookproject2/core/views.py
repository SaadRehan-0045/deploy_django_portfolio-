...
def update_settings(request):
    """Update portfolio settings - Fully Dynamic"""
    if request.method == 'POST':
        try:
...
def update_about(request):
    """Update about section - Personal Story and Image URL"""
    if request.method == 'POST':
        try:
            personal_story = request.POST.get('personal_story', '')
            about_img_url = request.POST.get('about_img_url', '')
            
            # Clean the text - remove <br> tags and extra spaces
            personal_story = personal_story.replace('<br>', '\n')  # Convert <br> to newlines
            personal_story = personal_story.replace('\\n', '\n')    # Convert \n to actual newlines
            personal_story = personal_story.strip()                # Remove extra spaces
            
            # Clean the image URL
            about_img_url = about_img_url.strip()
    ...
    if request.method == 'POST':
        try:
            ...
            project_data = {
                ...
                "image_url": convert_line_breaks(request.POST.get('image_url')),
